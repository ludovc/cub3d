...
static void	draw_borders(t_game *g, int cx, int cy)
{
	...
	draw_edge_if_needed(&g->img, bounds_check(g->map, cy - 1, cx), x0, y0, 1);
	draw_edge_if_needed(&g->img, bounds_check(g->map, cy + 1, cx), x0, y1 - MINIMAP_WALL_THICKNESS, 1);
	draw_edge_if_needed(&g->img, bounds_check(g->map, cy, cx - 1), x0, y0, 0);
	draw_edge_if_needed(&g->img, bounds_check(g->map, cy, cx + 1), x1 - MINIMAP_WALL_THICKNESS, y0, 0);
}
